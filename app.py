...
def rekomendasi_mobil():
    ...
    try:
        ...
        if not umur or not gaya or not pekerjaan or not manufacturer:
            response = make_response(jsonify({"error": "Semua field (umur, gaya, pekerjaan, produsen) harus diisi"}), 400)
            return add_cors_headers(response)
        ...
        rekomendasi = rekomendasi.format(umur=umur, gaya=gaya, pekerjaan=pekerjaan, manufacturer=manufacturer.capitalize())
